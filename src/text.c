...
void Text_Init() {
    unsigned char *atlas = malloc(128 * 8 * 8);
    for (int c = 0; c < 128; c++) {
        int cx = (c % 16) * 8;
        int cy = (c / 16) * 8;
        for (int row = 0; row < 8; row++) {
            ...
            for (int col = 0; col < 8; col++) {
                atlas[(cy + row) * 128 + (cx + col)] = (bits & (1 << col)) ? 255 : 0;
            }
        }
    }
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_ALPHA, 128, 64, 0, GL_ALPHA, GL_UNSIGNED_BYTE, atlas);
    free(atlas);
}
...
void Text_Draw(float x, float y, char *text, float size) {
    ...
    for (int i = 0; text[i]; i++) {
        ...
        float u0 = (int)(c % 16) * 8 / 128.0f;
        float v0 = (int)(c / 16) * 8 / 64.0f;
        float u1 = u0 + 8 / 128.0f;
        float v1 = v0 + 8 / 64.0f;
